...
const productSchema = new mongoose.Schema(
  ...
  validate: {
    validator: function (val) {
      ...
      // Mongoose query context hooks mein update matrix ko pull karein
      if (this.getUpdate) {
        const updateFields = this.getUpdate().$set || this.getUpdate();
        
        // Agar standard price bhi is query ke sath change ho rahi hai
        if (updateFields.price !== undefined) {
          return val < updateFields.price;
        }
      }

      // 4. Default safe fallback takki existing catalog entry crash na kare
      return true;
    },
